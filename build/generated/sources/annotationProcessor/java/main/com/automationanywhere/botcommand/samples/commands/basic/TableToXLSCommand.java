...
public final class TableToXLSCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(TableToXLSCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    TableToXLS command = new TableToXLS();
    ...
    if(convertedParameters.containsKey("outputFile")) {
      ...
      if (lastIndxDot == -1 || lastIndxDot >= filePath.length()) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","outputFile","xls"));
      }
      String fileExtension = filePath.substring(lastIndxDot + 1);
      if(!Arrays.stream("xls".split(",")).anyMatch(fileExtension::equalsIgnoreCase))  {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.FileExtension","outputFile","xls"));
      }
